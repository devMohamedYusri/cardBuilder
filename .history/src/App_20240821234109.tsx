...
const App = () => {
 ...
 const open=()=> {
  ...
  const onCancel=()=>{setProduct(defaultProduct) }
  ...
  const renderInputList = fromInput.map((item) =>
    <div className="flex flex-col w-full" key={item.id}>
      ...
      <Input type={item.type} name={item.name} id={item.id} value={productList[item.name]} onChange={onChangeHandler} />
    </div>
  );
  return (
      ...
      <Modal isOpen={isOpen} closeModel={close} title="Add Product">
        <form className="w-full" onSubmit={onsubmitHandler}>
          ...
          <div className="flex items-center space-x-2 mt-4">
            ...
            <Button classes="bg-black-100 h-10 hover:bg-gray-700 " width="w-full" ={onCancel}>
              cancel
            </Button>
          </div>
        </form>
