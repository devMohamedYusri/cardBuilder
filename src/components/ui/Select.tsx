...
export default function SelectCategory({selected,setSelected}:IProps) {
    return (
        <Listbox value={selected} onChange={setSelected}>
            <Label className="block text-sm font-medium leading-6 text-slate-200">Category</Label>
            <div className="relative mt-2">
                <ListboxButton className="relative w-full cursor-default rounded-md 
                bg-white py-1 pl-3 pr-10 text-left text-blue-500 shadow-sm ring-1 ring-inset 
                ...
                <ListboxOptions
                    transition
                    className="absolute z-10 mt-1 max-h-56 w-full overflow-auto rounded-md bg-white 
                    ...
                    {categories.map((category) => (
                        ...
                        >
                            <div className="flex items-center">
                                <img alt="" src={category.avatar} className="h-5 w-5 flex-shrink-0 rounded-full" />
